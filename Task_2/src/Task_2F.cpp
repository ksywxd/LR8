...
LibraryBook* createLibrary(int& size) {
    ...
    for (int i = 0; i < size; i++) {
        ...
        if (cin.fail()) { // Если ввели больше 19 символов
            cin.clear();
            cin.ignore(numeric_limits<streamsize>::max(), '\n');
        }
...
void addBook(LibraryBook*& arr, int& size) {
    ...
    cout << "Reg number: ";
    cin.getline(newArr[size].regNumber, 20);
    if (cin.fail()) {
        cin.clear();
        cin.ignore(numeric_limits<streamsize>::max(), '\n');
    }
    ...
    cout << "Author: ";
    cin.getline(newArr[size].author, 50);
    if (cin.fail()) {
        cin.clear();
        cin.ignore(numeric_limits<streamsize>::max(), '\n');
    }
    ...
    cout << "Title: ";
    cin.getline(newArr[size].title, 100);
    if (cin.fail()) {
        cin.clear();
        cin.ignore(numeric_limits<streamsize>::max(), '\n');
    }
    ...
    cout << "Year: ";
    newArr[size].year = (int)getValidNum();

    cout << "Publisher: ";
    cin.getline(newArr[size].publisher, 50);
    if (cin.fail()) {
        cin.clear();
        cin.ignore(numeric_limits<streamsize>::max(), '\n');
    }
...
void deleteBook(LibraryBook*& arr, int& size) {
    ...
    int idx = -1;
    for (int i = 0; i < size; i++)
        if (strcmp(arr[i].regNumber, reg) == 0)
        {
            idx = i;
            break;
        }
